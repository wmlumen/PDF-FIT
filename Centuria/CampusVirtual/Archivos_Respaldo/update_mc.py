...
# Define the new questions for Class 1
mc_questions = """
    let questions_{c['id']} = [];
    if ({c['id']} === 1) {{
        questions_{c['id']} = [
...
new_quiz_html_and_js = """<div class="section-card border-ej">
...
# We need to replace the old Autoevaluacion Secuencial (from <div class="section-card border-ej"> up to </script>)
# Because the old version is used for both 'ejercitario' and 'evaluacion_multiple_choice', we'll replace the regex pattern.
pattern = re.compile(r'<div class="section-card border-(?:ej|rubrica)">\s*<div class="section-title text-(?:ej|rubrica)">.*?<i class="bi bi-controller"></i> \d+\. Autoevaluación Secuencial.*?</script>', re.DOTALL)

# Since we injected this TWICE (once for ejercitario, once for evaluacion_multiple_choice) in the file, we just replace both.
new_content = pattern.sub(new_quiz_html_and_js.replace('\\', '\\\\'), content)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Quiz engine updated to support Multiple Choice.")
