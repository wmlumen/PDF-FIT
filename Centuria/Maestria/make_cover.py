...
def create_template(version=7):
    ...
    if os.path.exists('Logo Centuria.png'):
        run = p.add_run()
        run.add_picture('Logo Centuria.png', width=Inches(2.0))
    ...
    for i, (col1, col2, col3) in enumerate(data):
        ...
        trHeight.set(qn('w:val'), '600')  # Twips
    ...
    # The signature row should be tall
    table2.rows[1].height = Inches(1.0)
    ...
    for i in range(3):
        ...
        cell.text = "FIRMA Y SELLO"
        # vertical align to bottom is hard in docx, so we add newlines
        p = cell.paragraphs[0]
        p.text = "\n\n\nFIRMA Y SELLO"
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        for r in p.runs:
    ...
    c1 = table3.cell(0, 0)
    c1.text = "COMPROBANTE DE INGRESO Nº:"
    p = c1.paragraphs[0]
    p.runs[0].bold = True; p.runs[0].font.size = Pt(10); p.runs[0].font.name = 'Arial'
    ...
    # Actually docx sections apply to page breaks
